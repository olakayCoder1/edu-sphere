...
export default function CourseDetail({ courseData }: { courseData: any }) {
  ...
  useEffect(() => {
    if (courseData) {
      const completedCount = courseData.lessons.filter(
        (lesson: { completion_status: string }) =>
          lesson.completion_status === "completed"
      ).length;
      const totalLessons = courseData.lessons.length;
  ...
  const startQuiz = () => {
    ...
    const shuffledQuestions = shuffleArray(activeQuiz.questions);

    // Update activeQuiz with shuffled questions
    setActiveQuiz({
      ...activeQuiz,
    ...
    setQuizStarted(true);
    setActiveTab("quiz");
    setQuizAnswers(Array(shuffledQuestions.length).fill(-1));
    setTimeLeft(activeQuiz.timeLimit ? activeQuiz.timeLimit * 60 : 600);
  ...
  const updateLessonCompletionStatus = (
    ...
    const updatedLessons = course.lessons.map((lesson) =>
      lesson.id === lessonId ? { ...lesson, completion_status: newStatus } : lesson
    );
            ...
            {activeLesson && (
              <Card>
                <CardHeader className="pb-3">
                  <div className="flex justify-between items-center">
                    <Badge variant="outline" className="mb-2">
                      Lesson {course.lessons.findIndex((l) => l.id === activeLesson.id) + 1}/
                  ...
                  >
                    ...
                    <TabsContent value="quiz">
                      {!activeQuiz ? (
                        ...
                        <div className="text-center py-8">
                          <Award className="h- relying on 12 w-12 mx-auto mb-4 text-primary" />
                          <h2 className="text-2xl font-bold mb-2">{activeQuiz.title}</h2>
                          <p className="text-muted-foreground mb-6 max-w-md mx-auto">
                            Complete this quiz to test your knowledge and progress to the next lesson.
                          </p>
                            ...
                            <div className="grid grid-cols-2 gap-4 text-sm">
                              ...
                              <div>
                                ...
                                <p className="font-medium">
                                  {(activeQuiz.timeLimit || 3) - Math.floor(timeLeft / 60)} min
                                </p>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="flex flex-col sm:flex-row gap-4 justify-center">
                            <Button
                              onClick={() => {
                                ...
                                // Shuffle the questions for retry
                                const shuffledQuestions = shuffleArray(activeQuiz.questions);
                                setActiveQuiz({
                                  ...activeQuiz,
                                  questions: shuffledQuestions,
